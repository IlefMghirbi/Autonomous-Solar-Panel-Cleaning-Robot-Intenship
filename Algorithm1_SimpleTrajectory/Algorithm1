...
# Total points in one sweep = 11 (to be discussed)

###################     PARAMETERS INITIALIZATION   #########################

Robot_Width = 650 
Actual_Robot_Width = 650
...
def Algorithm1(borders, Robot_Width, Robot_Length, Panel_Width, Panel_Length, Interpolation_Step, U1,U2): #borders is the 2-D array [P0[x0,y0],P1[x1,y1],P2[x2,y2],P3[x3,y3]]
    ...
    for i in range (Nbr_sections):
        Start_Points_Array[0][i]= (Robot_Width*i) + (Actual_Robot_Width/2)
        Start_Points_Array[1][i]= borders[1][0]

    #create an array of end points (Xi, Ymax)   UPPER POINTS
    for i in range (Nbr_sections):
        End_Points_Array[0][i]= (Robot_Width*i) + (Actual_Robot_Width/2)
        End_Points_Array[1][i]= borders[1][1]
...
def main():
    ...
    borders = np.array([[325,325, 35000-325, 35000-325],[342,40000-342,40000-342,342 ]])
    ...
    #sections = divmod(Panel_Width, Robot_Width)
    #Nbr_sections = int(sections[0])
    #Non_Swept = sections[1]

    Nbr_sections, Robot_Width = section_divider(Panel_Width, Robot_Width)
    ...
    Trajectory = Algorithm1(borders, Robot_Width, Robot_Length, Panel_Width, Panel_Length,
                      Interpolation_Step, U1, U2)
   
    plt.rcParams["figure.autolayout"] = True

    file_path = "/home/ilef/PSolar_ws/src/scripts/src/trajectory_targets.json"
    trajectory_x = Trajectory[0].tolist()
    trajectory_y = Trajectory[1].tolist()
    coordinates = list(zip(trajectory_x, trajectory_y))
    serialized_trajectory = json.dumps(coordinates)


    with open(file_path,'w') as f:
        json.dump(serialized_trajectory, f, indent=4)
